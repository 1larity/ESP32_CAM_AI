...
class CameraCanvas(QtWidgets.QWidget):
    ...
    def zoom_delta(self, factor: float, anchor: QtCore.QPointF=None):
        ...
        self._fit_mode = False
        old_zoom = self._zoom
        self._zoom = max(0.10, min(4.0, self._zoom * factor))
    ...
    def wheelEvent(self, e: QtGui.QWheelEvent):
        ...
        if e.modifiers() & QtCore.Qt.KeyboardModifier.ControlModifier:
            ...
            self.zoom_delta(factor, QtCore.QPointF(e.pos()))
        else:
            super().wheelEvent(e)

    def mousePressEvent(self, e):
        if e.button() in (QtCore.Qt.MouseButton.LeftButton, QtCore.Qt.MouseButton.MiddleButton):
...
class CameraWidget(QtWidgets.QWidget):
    ...
    def __init__(self, cfg: CameraConfig, parent=None):
        ...
        self.label = QtWidgets.QLabel('Connecting…')  # legacy fallback, not shown
        ...
        act_zoom_in = btns.addAction('+')
        act_zoom_out = btns.addAction('−')
        ...
        act_100.triggered.connect(self._zoom_100)
    ...
    def _update_view_status(self):
        try:
            z = int(round(getattr(self.canvas, '_zoom', 1.0) * 100))
            px = int(getattr(self.canvas, '_pan', QtCore.QPointF(0,0)).x())
            py = int(getattr(self.canvas, '_pan', QtCore.QPointF(0,0)).y())
            mode = 'Fit' if getattr(self.canvas, '_fit_mode', True) else f"{z}%"
            self.lbl_status.setText(f"{mode}  pan {px},{py}")
        except Exception:
            pass
