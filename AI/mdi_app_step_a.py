...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Events dock visibility toggle
        self.act_view_events = menu_view.addAction('Events Panel')
        self.act_view_events.setCheckable(True); self.act_view_events.setChecked(True)
        self.act_view_events.toggled.connect(self.eventsDock.setVisible)
        try:
            self.eventsDock.visibilityChanged.connect(self.act_view_events.setChecked)
        except Exception:
            pass
        # Recognition overlays for active camera
        rec_menu = menu_view.addMenu('Recognition Overlays')
        self.act_view_yolo = rec_menu.addAction('YOLO'); self.act_view_yolo.setCheckable(True); self.act_view_yolo.setChecked(True)
        self.act_view_face = rec_menu.addAction('Faces'); self.act_view_face.setCheckable(True); self.act_view_face.setChecked(True)
        self.act_view_dogid = rec_menu.addAction('Dog ID'); self.act_view_dogid.setCheckable(True); self.act_view_dogid.setChecked(True)
        self.act_view_yolo.toggled.connect(self._apply_overlay_menu_to_active)
        self.act_view_face.toggled.connect(self._apply_overlay_menu_to_active)
        self.act_view_dogid.toggled.connect(self._apply_overlay_menu_to_active)

        # Tools dock (controls act on active camera)
        self.toolsDock = QtWidgets.QDockWidget('Tools', self)
        toolw = QtWidgets.QWidget()
        tl = QtWidgets.QHBoxLayout(toolw); tl.setContentsMargins(6,6,6,6)
        btn_start = QtWidgets.QPushButton('Start'); btn_stop = QtWidgets.QPushButton('Stop')
        btn_rec = QtWidgets.QPushButton('Start Rec'); btn_stoprec = QtWidgets.QPushButton('Stop Rec')
        tl.addWidget(btn_start); tl.addWidget(btn_stop); tl.addWidget(btn_rec); tl.addWidget(btn_stoprec); tl.addStretch(1)
        self.toolsDock.setWidget(toolw)
        self.addDockWidget(QtCore.Qt.TopDockWidgetArea, self.toolsDock)
        def _active_cam():
            sub = self.mdi.activeSubWindow()
            if not sub: return None
            w = sub.widget()
            return w if isinstance(w, CameraWidget) else None
        btn_start.clicked.connect(lambda: (_active_cam() and _active_cam().start_stream()))
        btn_stop.clicked.connect(lambda: (_active_cam() and _active_cam().stop_stream()))
        btn_rec.clicked.connect(lambda: (_active_cam() and _active_cam().start_recording()))
        btn_stoprec.clicked.connect(lambda: (_active_cam() and _active_cam().stop_recording()))
        # Log dock
        self.logDock = QtWidgets.QDockWidget('Log', self)
        self.logView = QtWidgets.QTextEdit(); self.logView.setReadOnly(True)
        self.logDock.setWidget(self.logView)
        self.addDockWidget(QtCore.Qt.BottomDockWidgetArea, self.logDock)
        self.act_view_log = menu_view.addAction('Log Panel')
        self.act_view_log.setCheckable(True); self.act_view_log.setChecked(True)
        self.act_view_log.toggled.connect(self.logDock.setVisible)
        try:
            self.logDock.visibilityChanged.connect(self.act_view_log.setChecked)
        except Exception:
            pass
    ...
    def on_sub_activated(self, sub):
        ...
        if isinstance(w, CameraWidget):
            ...
            try:
                ...
                w.eventLogged.connect(self._append_log)
            except Exception:
                pass
            # Sync View→Recognition menu from active camera state
            try:
                self.act_view_yolo.setChecked(w.chk_yolo.isChecked())
                self.act_view_face.setChecked(w.chk_face.isChecked())
                self.act_view_dogid.setChecked(w.chk_dogid.isChecked())
            except Exception:
                pass
    ...
    def _apply_overlay_menu_to_active(self):
        try:
            sub = self.mdi.activeSubWindow()
            w = sub.widget() if sub else None
            if isinstance(w, CameraWidget):
                # sync from menu to widget toggles
                try: w.chk_yolo.setChecked(self.act_view_yolo.isChecked())
                except Exception: pass
                try: w.chk_face.setChecked(self.act_view_face.isChecked())
                except Exception: pass
                try: w.chk_dogid.setChecked(self.act_view_dogid.isChecked())
                except Exception: pass
        except Exception:
            pass
