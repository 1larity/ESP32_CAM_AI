...
- **Generated**: 2025-11-08 12:00:51 +0000
...
class CameraRecorder:
    ...
    def _sanitize(name: str) -> str:
        ...
        return "".join(keep)[:64]
## `detection_packet.py`
**Absolute path**: C:\Users\stellaris\Documents\PlatformIO\Projects\ESP32_CAM_AI\AI\detection_packet.py
**Size**: 521 bytes
**Modified**: 2025-11-08 00:14:26 +0000
**SHA256**: 8df61cc2a637994f4407e1b7fc8957f6aff59c52e9da959ef2ebb160b38d6660
``````python# detection_packet.py
...
**Size**: 8775 bytes
**Modified**: 2025-11-08 10:06:17 +0000
**SHA256**: ad80da4aac24e606645a27cd1888cfe497870d92e0495da768be1b271929a360
``````python# detectors.py
...
class DetectorConfig:
    ...
    def from_app(cls, app_cfg):
        ...
        return cls(
            ...
            yolo_nms=0.45,
            interval_ms=app_cfg.detector_interval_ms,
        ...
        while not self._stop.is_set():
            ...
            if self._net is not None:
                try:
                    ...
                    for (cx, cy, ww, hh), score, cid in zip(boxes, scores, ids):
                        ...
                        pkt.yolo.append(DetBox(COCO_ID_TO_NAME[cid], float(score), (x1, y1, x2, y2)))
                except Exception:
                    pass
            ...
            if self._face is not None:
                try:
                    ...
                    for (fx, fy, fw, fh) in faces:
                        name = "face"
                        score = 1.0
                        # **CHANGED: LBPH tuning (equalizeHist + accept_conf)**
                        if self._rec is not None:
                            try:
                                ...
                                # LBPH: lower conf is better. Tune accept_conf empirically.
                                accept_conf = 95.0
                                if conf <= accept_conf and lab in self._labels:
                                    name = self._labels[lab]
                                    # Map LBPH conf to 0..1 score for display
                                    score = float(max(0.0, min(1.0, 1.0 - (conf / accept_conf))))
                            except Exception:
                                pass
...
**Size**: 4883 bytes
**Modified**: 2025-11-08 11:55:09 +0000
**SHA256**: 7711b824a3c17d4e44aaf5a9577001eabf60d97ef0ac0c60f949283bbbbfe0f0
...
## `face_params.py`
**Absolute path**: C:\Users\stellaris\Documents\PlatformIO\Projects\ESP32_CAM_AI\AI\face_params.py
**Size**: 1150 bytes
**Modified**: 2025-11-08 00:41:48 +0000
**SHA256**: 2f19560c6f54a44d2f7f70bbc53dc928b912b5de17e7a0ba331ce84dcfb053e1
``````python# face_params.py
from __future__ import annotations
...
## `face_tuner.py`
**Absolute path**: C:\Users\stellaris\Documents\PlatformIO\Projects\ESP32_CAM_AI\AI\face_tuner.py
**Size**: 2701 bytes
**Modified**: 2025-11-08 00:42:21 +0000
**SHA256**: 1c1498fe33c39af6f171d76aab5e69d02da5f810813b939132013d72c8e01687
``````python# face_tuner.py
from __future__ import annotations
...
**Size**: 11984 bytes
**Modified**: 2025-11-08 11:55:07 +0000
**SHA256**: 0be053f1ee9895769eac2c28637b6c3106c022463f63315254e33d69735bd3fc
``````python# mdi_app.py
...
class GraphicsView(QtWidgets.QGraphicsView):
    ...
    def wheelEvent(self, e: QtGui.QWheelEvent):
        if QtWidgets.QApplication.keyboardModifiers() == QtCore.Qt.KeyboardModifier.ControlModifier:
            ...
            t = self.mapToScene(e.position().toPoint())
            self.setTransformationAnchor(QtWidgets.QGraphicsView.ViewportAnchor.NoAnchor)
            self.setResizeAnchor(QtWidgets.QGraphicsView.ViewportAnchor.NoAnchor)
...
# -----------------------------------------------------------------------------
class CameraWidget(QtWidgets.QGraphicsView):
    def __init__(self, cam_cfg: CameraSettings, app_cfg: AppSettings, parent=None):
        ...
        self._recorder = PrebufferRecorder(app_cfg)
        self._presence = PresenceBus.instance()
        self._detector = None
        self._capture = None
        self._ptz = None
        self._init_detector()
    ...
    def _init_detector(self):
        cfg = DetectorConfig.from_app(self.app_cfg)
        self._detector = DetectorThread(cfg, self.cam_cfg.name)
        self._detector.resultsReady.connect(self._on_detections)
        self._detector.start()

    def start(self):
        if self.cam_cfg.url:
            self._capture = StreamCapture(self.cam_cfg.url, self._on_frame)
            self._capture.start()
        if self.cam_cfg.has_ptz:
            self._ptz = PTZClient(self.cam_cfg)
    ...
    def _on_frame(self, bgr, ts_ms):
        self._recorder.add_frame(bgr, ts_ms)
        self._detector.submit_frame(self.cam_cfg.name, bgr, ts_ms)
    ...
    def _on_detections(self, pkt: DetectionPacket):
        if not pkt or not pkt.faces and not pkt.yolo:
            return
        self._presence.on_detection(pkt)
        img = draw_overlays(pkt, self._pixmap_item.pixmap(), self._overlays)
        self._pixmap_item.setPixmap(img)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, app_cfg: AppSettings):
       ...
       # self.events_pane = EventsPane(self)
    ...
    def _add_camera_url_dialog(self):
        ...
        if ok and text:
            cam_cfg = CameraSettings(name=f"Custom-{len(self.app_cfg.cameras)+1}", url=text)
    ...
    # -----------------------------------------------------------------
    # Added: rebuild faces (used by new Tools → Rebuild face model)
    # -----------------------------------------------------------------
    def _rebuild_faces(self):
        svc = EnrollmentService.instance()
        ok = False
        try:
            ok = svc.rebuild_from_disk()
        except Exception as e:
