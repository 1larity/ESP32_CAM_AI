Refactor for readability and maintainability preferably no file should have more than 150 lines. Prefer pure functions, remove dead code, add type hints where beneficial, keep behavior identical. Before starting analyse the files and suggest one of the larger ones to functionally decompose. Consider isolation of responablilties, keep it modular, use folders to group UI, AI, Enrollment,recording, camera ,util functions.
