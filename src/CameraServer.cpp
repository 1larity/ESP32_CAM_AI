...
static void renderCameraPage(AsyncWebServerRequest* request) {
  ...
  // Small nav: Wi‑Fi Settings (lands on AP page when device is in AP mode)
  html += "<div class='nav'>"
          "<a class='btn' href='/'>Wi‑Fi Settings</a>"
          "</div>";
...
void startCameraServer() {
  ...
  camServer.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    // In STA mode this is the camera UI; in AP mode WiFiManager owns "/"
