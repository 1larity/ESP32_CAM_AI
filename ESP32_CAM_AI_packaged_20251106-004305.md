...
- **Generated**: 2025-11-06 00:43:05 +0000
...
## `mdi_app_full_step_bc_fix1.py`
**Absolute path**: C:\Users\stellaris\Documents\PlatformIO\Projects\ESP32_CAM_AI\AI\mdi_app_full_step_bc_fix1.py
**Size**: 96062 bytes
**Modified**: 2025-11-06 00:41:46 +0000
**SHA256**: 32265dc0ae312afd6bbabf1c920c30cdce5fcb3da58e064cc60c705a4590299d
``````python#!/usr/bin/env python3
"""
...
class _VideoView(QtWidgets.QLabel):
    """Pan/zoom image view for a single camera.
    Ctrl+Wheel: zoom about cursor. Right-drag: pan. Fit and 100% modes.
    """
    ...
    def zoom_in(self):
        self._fit = False
        self._scale = min(4.0, self._scale * 1.15)
        self.update()

    def zoom_out(self):
        self._fit = False
        self._scale = max(0.10, self._scale / 1.15)
        self.update()
    ...
    def wheelEvent(self, ev: QtGui.QWheelEvent):
        ...
        factor = 1.15 ** angle
        self._scale = float(max(0.10, min(4.00, self._scale * factor)))
        # keep cursor point stable
        if old_scale != 0 and self._img is not None:
            p = ev.position()
            vw, vh = self.width(), self.height()
            iw, ih = self._img.width(), self._img.height()
            vx = (p.x() - vw * 0.5) - self._pan.x()
            vy = (p.y() - vh * 0.5) - self._pan.y()
            ix = vx / old_scale + iw * 0.5
            iy = vy / old_scale + ih * 0.5
            nx = (ix - iw * 0.5) * self._scale
            ny = (iy - ih * 0.5) * self._scale
            self._pan = QtCore.QPointF((p.x() - vw * 0.5) - nx, (p.y() - vh * 0.5) - ny)
        self.update()
    ...
    def mouseMoveEvent(self, ev: QtGui.QMouseEvent):
        if self._dragging and not self._fit:
            ...
            self._pan += QtCore.QPointF(d.x(), d.y())
            self._last_pos = ev.pos()
            self.update()
        else:
    ...
    def paintEvent(self, ev: QtGui.QPaintEvent):
        ...
        if self._fit:
            ...
            return
        iw, ih = self._img.width(), self._img.height()
        sw, sh = int(iw * self._scale), int(ih * self._scale)
        if sw < 2 or sh < 2:
            return
        pix = QtGui.QPixmap.fromImage(self._img).scaled(sw, sh, QtCore.Qt.IgnoreAspectRatio, QtCore.Qt.SmoothTransformation)
        x = int(self.width() * 0.5 + self._pan.x() - sw * 0.5)
        y = int(self.height() * 0.5 + self._pan.y() - sh * 0.5)
        p.drawPixmap(x, y, pix)

    def map_image_point_to_view(self, pt: QtCore.QPointF) -> QtCore.QPointF:
        """Map an image-space point (x,y in pixels of the source frame) to this widget's view coords."""
        if self._img is None or self._img.isNull():
        ...
        if self._fit:
            ...
            x0 = (vw - sw) * 0.5
            y0 = (vh - sh) * 0.5
            return QtCore.QPointF(x0 + pt.x() * s, y0 + pt.y() * s)
        # zoomed / 100% with pan
        sw, sh = iw * self._scale, ih * self._scale
        x = vw * 0.5 + self._pan.x() - sw * 0.5 + pt.x() * self._scale
        y = vh * 0.5 + self._pan.y() - sh * 0.5 + pt.y() * self._scale
...
class FaceDB:
    ...
    def load(self):
        ...
        for name in sorted(os.listdir(self.base)):
            ...
            lid+=1
        self.views=[None]*lid
        for n,i in label_map.items(): self.views[i]=n
        # LBPH
    ...
    def recognize_roi(self, gray_roi):
        ...
        if self.model is not None:
            try:
                ...
                if 0<=pred<len(self.views) and dist<=95.0:
                    return self.views[pred], max(0.0,min(1.0,1.0-(dist/95.0)))
            except Exception:
                pass
        # ORB fallback
...
class CameraWidget(QtWidgets.QWidget):
    ...
    def __init__(self, cfg: CameraConfig, parent=None):
        ...
        # moved to _VideoView alignment
        # handled by _VideoView minimum size
        # style handled by _VideoView
        ...
        self._rec_last_ts: Optional[float] = None
        self._rec_frames: int = 0
        self._rec_start_ts: Optional[float] = None
        
        # PTZ config and flags
        self.ptz_enabled = False
        self.ptz_deadzone_px = 12
        self.ptz_px_to_servo = 0.02
        self.ptz_max_step_deg = 2.0
        self.ptz_invert_pan = False
        self.ptz_invert_tilt = False
        ...
        self.log_jsonl_path = os.path.join(self.log_dir, f"events_{safe_name}_{safe_host}.jsonl")
        ...
        act_fit.triggered.connect(self.view.set_fit)
        act_100.triggered.connect(self.view.set_one_to_one)
    ...
    def start_recording(self):
        ...
        try:
            with open(self.log_jsonl_path, 'a', encoding='utf-8') as f:
                ...
                f.write(_json.dumps({'type':'recording','action':'start','camera':self.cfg.name,'host':self.cfg.host,'start':self._rec_start_ts}, ensure_ascii=False)+'\n')
        ...
        try:
            if self._rec_start_ts is not None:
                ...
                event = {
                    ...
                    'host': self.cfg.host,
                    'start': self._rec_start_ts,
                    'end': end_ts,
                    'frames': int(self._rec_frames),
                ...
                # human friendly log
                dur = end_ts - self._rec_start_ts
                self._log_event(f"Recording stopped. frames={self._rec_frames} dur={dur:.1f}s")
        except Exception:
            pass
        self.recording = False
        self.writer = None
    ...
    def current_size(self) -> Tuple[int, int]:
        """Return (H, W) of the most recent frame or view image, else default."""
        with self._frame_lock:
            if self._latest_frame is not None:
                h, w = self._latest_frame.shape[:2]
                return h, w

        # fallback to view's QImage size
        try:
            if (
                hasattr(self, "view")
                and self.view is not None
                and getattr(self.view, "_img", None) is not None
                and not self.view._img.isNull()
            ):
                return self.view._img.height(), self.view._img.width()
        except Exception:
            pass
...
def on_frame(self, bgr: np.ndarray, ts: float):
    ...
    # Keep a clean copy before overlays for saving samples
    raw_bgr = bgr.copy()
    ...
    # Recording write with FPS cap
    if self.recording and self.writer is not None:
        if (self._rec_last_ts is None) or (ts - self._rec_last_ts) >= (1.0 / max(1e-3, self.target_fps)):
            W = int(self.writer.get(cv2.CAP_PROP_FRAME_WIDTH)) or bgr.shape[1]
            H = int(self.writer.get(cv2.CAP_PROP_FRAME_HEIGHT)) or bgr.shape[0]
            W -= (W % 2); H -= (H % 2)
            if bgr.shape[1] != W or bgr.shape[0] != H:
                try:
                    out_frame = cv2.resize(bgr, (W, H))
                except Exception:
                    out_frame = bgr
            else:
                out_frame = bgr
    ...
    # Best-effort enroll is handled in on_frame collection path; refresh model
    try:
        self.facedb.load()
        self.lbl_status.setText(f'Enrolled face: {name} (model refreshed)')
    except Exception:
        self.lbl_status.setText(f'Face enrolled: {name}')
        return
    if name is None:
            name=self.ed_name.text().strip() or 'person'
            x,y,w,h=self.last_face_bbox
    ...
    def aim_at_target(self):
        """PTZ aim loop: pick primary target from tracker and nudge servos."""
    if not getattr(self, "ptz_enabled", False):
        return
    # Choose primary target from tracker
    pt = None
    try:
        pt = self.tracker.primary_target(prefer='cat')
    except Exception:
        pt = None
    if pt is None or self.view is None or self.view._img is None or self.view._img.isNull():
        return
    x, y, w, h, cls, _ = pt
    cx = x + w * 0.5
    cy = y + h * 0.5
    tgt_view = self.view.map_image_point_to_view(QtCore.QPointF(cx, cy))
    ctr_view = self.view.view_center()
    dx = tgt_view.x() - ctr_view.x()
    dy = tgt_view.y() - ctr_view.y()
    dead_px = getattr(self, "ptz_deadzone_px", 12)
    if abs(dx) < dead_px and abs(dy) < dead_px:
        return
    px_to_servo = getattr(self, "ptz_px_to_servo", 0.02)
    max_step = getattr(self, "ptz_max_step_deg", 2.0)
    pan_delta = max(-max_step, min(max_step, dx * px_to_servo))
    tilt_delta = max(-max_step, min(max_step, dy * px_to_servo))
    if getattr(self, "ptz_invert_pan", False):
        pan_delta = -pan_delta
    if getattr(self, "ptz_invert_tilt", False):
        tilt_delta = -tilt_delta
    try:
        self._ptz_nudge(pan_delta, tilt_delta)
    except Exception as e:
        print(f"[PTZ] aim error: {e}")

        # Compute target center in image coordinates
        x, y, w, h = det.get('bbox', (0, 0, 0, 0))
        cx = x + w * 0.5
        cy = y + h * 0.5

        # Map to view coords using the current zoom/pan/fit
        tgt_view = self.view.map_image_point_to_view(QtCore.QPointF(cx, cy))
        ctr_view = self.view.view_center()

        dx = tgt_view.x() - ctr_view.x()   # +dx means target to the right
        dy = tgt_view.y() - ctr_view.y()   # +dy means target below

        # Dead-zone and rate limit
        dead_px = getattr(self, "ptz_deadzone_px", 12)
        if abs(dx) < dead_px and abs(dy) < dead_px:
            return

        # Map pixels to servo delta using configured scale
        # Positive dx -> pan right, positive dy -> tilt down
        px_to_servo = getattr(self, "ptz_px_to_servo", 0.02)  # deg per pixel (example)
        max_step = getattr(self, "ptz_max_step_deg", 2.0)     # clamp per tick

        pan_delta = max(-max_step, min(max_step, dx * px_to_servo))
        tilt_delta = max(-max_step, min(max_step, dy * px_to_servo))

        # Optional invert flags
        if getattr(self, "ptz_invert_pan", False):
            pan_delta = -pan_delta
        if getattr(self, "ptz_invert_tilt", False):
            tilt_delta = -tilt_delta

        # Dispatch to ESP32-CAM PTZ endpoint
        try:
            self._ptz_nudge(pan_delta, tilt_delta)
        except Exception as e:
            print(f"[PTZ] aim error: {e}")
